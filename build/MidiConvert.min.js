...
f.setInstrument("percussion"===a.transport._instruments[d]?9:9<=d?d+1:d,a.transport._instruments[d]);a.transport.timeSignature&&f.setTimeSignature(a.transport.timeSignature[0],a.transport.timeSignature[1]);c.map(N).reduce(I,[]).sort(M).reduce(L,[]).reduce(O,[]).reduce(K,f)});return b.toBytes()},MidiGen:J,parse:function(a,b){var c=w(a);0===c.header.formatType&&(c.tracks=W(c.tracks[0]));var d=c.header.ticksPerBeat;a=[];var f=!1,h,g,k,m,p,q,l,e;b=b||{};b.PPQ="undefined"===typeof b.PPQ?128:b.PPQ;b.noteName=
"undefined"===typeof b.noteName?!0:b.noteName;b.duration="undefined"===typeof b.duration?!0:b.duration;b.velocity="undefined"===typeof b.velocity?!0:b.velocity;for(m=0;m<c.tracks.length;m++){h=c.tracks[m];g=[];k=0;b.duration&&x(h.reduce(Q,{})).some(R)&&(h=h.reduce(P));for(p=0;p<h.length;p++)if(l=h[p],k+=l.deltaTime,"noteOn"===l.subtype)q={_note:l.noteNumber,_ticks:k,midiNote:l.noteNumber,time:k},b.noteName&&(q.noteName=E(l.noteNumber)),b.velocity&&(q.velocity=l.velocity/127),g.push(q);else if("noteOff"===
l.subtype)for(q=g.length-1;0<=q;q--){if(e=g[q],e._note===l.noteNumber&&"undefined"===typeof e.duration){b.duration&&(e.duration=Math.round((k-e._ticks)/d*b.PPQ)+"i");e.time=Math.round(e.time/d*b.PPQ)+"i";delete e._note;delete e._ticks;break}}else if("meta"===l.type&&"trackName"===l.subtype)l=l.text,l.replace(/\u0000/g,"");else if(l.controllerType===n.CONTROLLER.DAMPER_PEDAL)if(64<=l.value&&!f)l={_ticks:k,eventName:"sustain",time:k},f=!0,g.push(l);else if(64>l.value&&f){for(q=g.length-1;0<=q;q--)if(e=
g[q],"sustain"===e.eventName&&"undefined"===typeof e.duration){b.duration&&(e.duration=Math.round((k-e._ticks)/d*b.PPQ)+"i");e.time=Math.round(e.time/d*b.PPQ)+"i";delete e._note;delete e._ticks;break}f=!1}b.deterministic&&(g=g.sort(S));0<g.length&&a.push(g)}d=c.tracks.reduce(I,[]);c=c.tracks.reduce(T,{_instrumentsMap:{},instrumentsMap:{}});c={_instruments:x(c._instrumentsMap),bpm:V(d),instruments:x(c.instrumentsMap),timeSignature:U(d)};return{parts:a,transport:c}}}});
